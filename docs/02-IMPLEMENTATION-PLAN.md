...
- [x] Resource エディター: レコード追加/編集/削除/並び替え、バリデーション(§16.3)、現在値との差分表示、raw preview、サイズ表示
- [x] 安全対策: 全レコード削除警告・最後の NS 削除警告・Name 再入力オプション(§16.4)
- [x] UPDATE 実行フロー(preview → 再認証 → 実行 → TXID 表示)
- [x] Renewal: 対象分類(§17.1)、個別 Renewal
- [x] 一括 Renewal: 逐次実行 + Name ごとの成功/失敗/スキップ結果、進捗表示(§17.2–17.3)
- [x] Transfer: アドレス検証 → Lockup 説明 → Name 再入力 → 再認証 → 実行(§18.1)
- [x] Transfer 状態追跡と Finalize(可能時期・残りブロック表示 §18.2–18.3)
- [x] Revoke: 危険操作領域に隔離、影響説明 → Name 手入力 → パスワード + TOTP → 最終確認(§19)
- [x] アプリ内通知: 監視ループから生成(Renewal 接近 / Finalize 可能 / Transfer 状態変化 / 接続断 §20.1)、しきい値設定(§17.4)。TX 確定・失敗通知と Wallet 同期遅延通知は Phase 5 送り(下記参照)
...
- [ ] Web Push — 見送り。§20.2 は Web Push を含む外部通知群を「初期版では任意機能」と位置付けており、ntfy/Discord Webhook で §20.2 の要件は満たされている。`push_subscriptions` テーブルは Phase 5 着手時に確保済みだが未使用。将来必要になれば再検討する。
...
- [x] 障害診断画面(接続・同期・バージョンの一括診断) — `GET /api/diagnostics` が `/api/status` のポーリングキャッシュ(最大30秒遅延)を経由せず、その場で hsd に直接問い合わせて node/wallet の到達性・バージョン・Network 一致・`computeDashboardWarnings()` を返す。接続情報は API キーを含まない `SafeConnection` のみ。Web の `/settings/diagnostics` ページで「今すぐ診断」実行可能。regtest 実機で疎通確認済み。
